...
test('that api/request-access can handle sufficient number of requests per minute (30s burst) (sequential)', function () {
    ...
    for ($i = 0; $metrics['responseTimeSum'] < 30.0; ++$i) {
        $payloadIndex = $i % $payloadCount;

        $startTime = hrtime(true);
        $response = Http::post('http://localhost:8000/api/request-access', [$payloads[$payloadIndex]]);
    ...
    print_benchmark_report($metrics, 'Requests_Per_Minute_Sequential', true);
...
test('that api/request-access can handle sufficient number of requests per minute (30s burst) (concurrent)', function () {
    $payloads = get_payload();
    $payloadCount = count($payloads);
    ...
    for ($i = 0; $metrics['responseTimeSum'] < 30.0; ++$i) {
        ...
        $startTime = hrtime(true);
        $responses = Http::pool(function (Pool $pool) use ($batchRequestCount, $payloads, $payloadCount, $metrics) {
            $requests = [];
            for ($j = 0; $j < $batchRequestCount; ++$j) {
                $payloadIndex = ($j + $metrics['requestCount']) % $payloadCount;

                $requests[] = $pool
                    ->withBody($payloads[$payloadIndex], 'text/plain')
    ...
    print_benchmark_report($metrics, 'Requests_Per_Minute_Concurrent', true);
